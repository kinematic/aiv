...
$this->title = 'Контакты';
$this->params['breadcrumbs'][] = ['label' => $site->sitename, 'url' => ['sites/sites/view', 'id' => $site->id]];
	...
	<div class='row'>
		...
		// 						]);
						...
						'urlCreator' => function ($action, $othermodel, $key, $index) use ($site) {
							if ($action === 'update') {
								$url ='index.php?r=address/contacts/update&id=' . $othermodel->id . '&siteID=' . $site->id;
								return $url;
							}

							if ($action === 'delete') {
								$url ='index.php?r=address/contacts/delete&id=' . $othermodel->id . '&siteID=' . $site->id;
								return $url;
							}
						}
					],
